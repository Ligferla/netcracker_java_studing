...
public interface DateCollections {
    ...
    /**
     * Sets a style of string representation of dates that is used by other
     * methods of the class.<br/>
     * If this method is NOT called, the default {@link DateFormat#MEDIUM} style
     * is used.
     *
     * @param dateStyle One of the style constants defined in the
     * {@link DateFormat} class
     */
    void setDateStyle(int dateStyle);

    /**
     * Parses the <code>dateString</code> (a date with no time) to {@link Date}
     * (with {@link DateFormat})) and converts it to {@link Calendar}.
     *
     * @param dateString string compatible with a style set by
     * {@link #setDateStyle(int)})
     ...
     * with {@link DateFormat} using a style set by
     * {@link #setDateStyle(int)}.<br/>
     * Note: the result depends on the system's Locale. E.g. if the locale is
     * en_US the example of the string is "Jan 15, 2015" (for style =
     * {@link DateFormat#MEDIUM})
     */
    String toString(Calendar date);

    /**
     * Creates a map and adds <code>elementsNumber</code> instances of
     * {@link Element} class to the map (key,element), where key is the String
     * representation of <code>birthDate</code> field of the element (use
     * {@link #toString(Calendar)}).<br/>
     * The parameters of an element's constructor must obey the following rules:
     * <dl>
     * <dt>birthDate</dt><dd>firstDate + i * 110 days (i is the index of the
     * element)</dd>
     * <dt>lifetime (days)</dt><dd>random uniformly distributed {@code int}
     * value between 0 (inclusive) and 2000 (exclusive)</dd>
     * </dl>
     * The created map must be stored as a field. Note: when implementing
     * <code>getMainMap, getMainList, getSortedSubMap</code> you can rely on the
     * fact that this method is called BEFORE that methods. Note 2: the work of
     * this method depends on the style set by {@link #setDateStyle(int)}.
     *
     * @param elementsNumber the number of elements to instantiate
     * @param firstDate birth date of the first element; the time fields
     * (hours/minutes/..) do not matter
     */
    void initMainMap(int elementsNumber, Calendar firstDate);

    /**
     * Sets the given map to the the main field of the class.<br/>
     * Note: this method exists in order to test other methods if
     * {@link #initMainMap(int, Calendar)} has incorrect implementation.
     */
     ...
     *
     * @return map, the main field of the class. Can be null if
     * <code>initMainMap</code> method was not called
     */
    Map<String, Element> getMainMap();

    /**
     * Returns those entries of the main map, where elements'
     * <code>birthDate</code> &gt; today (i.e. elements to be born in future:
     * not today and not in the past). The resulting sub-map must be sorted
     * according to the <code>birthDate</code> (ascending order of dates; but
     * it's not alphabetic natural order of keys!).
     *
    ...
    /**
     * Represents values of the main map as a {@link List}. The list must be
     * sorted according to the <code>birthDate</code> (ascending order).
     *
    ...
    /**
     * Sorts the given list according to the <code>deathDate</code> (ascending
     * order).<br/>
     * Don't use {@link List#sort(Comparator)} method because it's since Java 8
     * (and Java version in Skill Bench is 7 now). Use a method of
     * {@link Collections}.
     */
    void sortList(List<Element> list);

    /**
     * Removes some elements from the given list. The <code>birthDate</code> of
     * removing elements must be in winter (i.e. in December, January or
     * February).<br/>
    ...
    /**
     * A class for objects to store in the collections. Has two dates -
     * Calendars where the time fields (hours/minutes/..) do not matter.
     */
    public static class Element {
        ...
        public boolean equals(Object obj) {
            ...
            if (!equalFields(bd, Calendar.DATE)) {
                return false;
            }
            if (!equalFields(bd, Calendar.MONTH)) {
                return false;
            }
            if (!equalFields(bd, Calendar.YEAR)) {
                return false;
            }
            return true;
        }
